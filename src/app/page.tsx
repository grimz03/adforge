    ...
    return (
      <main className="app-shell">
        ...
        <div className="wizard-wrap">
          ...
          {step === 3 && <section className="preview-layout"><div className="card wizard-card"><div className="section-title"><div><label>Your ad preview</label><p className="muted">AI-generated copy based on what you told us.</p></div><span className="ai-pill">✦ AI READY</span></div><div className="ad-preview"><div className="preview-top"><span>{business || 'YOUR BUSINESS'}</span><span>SPONSORED</span></div><div className="creative"><div className="creative-mark">{adType === 'Sale / Promotion' ? 'SAVE' : 'NEW'}</div><h2>{offer || 'Your next customer is closer than you think.'}</h2><p>{business ? `${business} is bringing something special to ${location || 'your local community'}.` : 'Create a message that gets your local customers to stop, look, and act.'}</p><button>{adType === 'Event' ? 'Learn More' : 'Get Offer'}</button></div></div><div className="actions"><button className="secondary" onClick={() => setStep(2)}>← Back</button><button className="primary" onClick={() => alert(`Demo campaign ready: ${selected.name}`)}>Launch ${selected.price} Blast <span>→</span></button></div></div><aside className="card summary"><p className="eyebrow">CAMPAIGN SUMMARY</p><h3>{business || 'Your Business'}</h3><div className="summary-row"><span>Ad type</span><b>{adType}</b></div><div className="summary-row"><span>Location</span><b>{location || 'Local area'}</b></div><div className="summary-row"><span>Package</span><b>{selected.name}</b></div><div className="total"><span>Total</span><b>${selected.price}</b></div><p className="fine">No subscription required. You'll review your campaign before any charge is made.</p></aside></section>}
        </div>
      </main>
    );
  }
